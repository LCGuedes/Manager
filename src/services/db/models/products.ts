...
export const insertProductInProductsTable = (
  ...
  db.transaction((tx) => {
    tx.executeSql(
      query.insert_into_products_table,
      params,
      (tx, results) => {
        getResults(results);
      },
...
export const selectProductsFromProductsTable = (
  clientName: string,
  getResults: any
) => {
...
export const updateProductInProductsTable = (editedProduct: productType) => {
  ...
  db.transaction((tx) =>
    tx.executeSql(
      query.update_product_from_products_table,
      params,
      (tx, results) => {}
    )
  );
...
export const DeleteProductInProductsTable = (editedProduct: productType) => {
  ...
  db.transaction((tx) => {
    tx.executeSql(
      query.delete_product_from_products_table,
      params,
      (tx, results) => {}
    );
  });
